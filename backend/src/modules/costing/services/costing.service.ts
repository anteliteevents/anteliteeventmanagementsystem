...
class CostingService {
   ...
   * Get cost by ID
   */
  async getCostById(costId: string): Promise<Cost | null> {
   ...
   * Duplicate cost
   */
  async duplicateCost(costId: string, data?: {
    eventId?: string;
    description?: string;
  }): Promise<Cost> {
  ...
  async deleteBudget(budgetId: string): Promise<void> {
    ...
    await pool.query(query, [budgetId]);
   ...
   * Duplicate budget
   */
  async duplicateBudget(budgetId: string, data?: {
    eventId?: string;
    category?: string;
  }): Promise<Budget> {
    ...
    return await this.setBudget(
      data?.eventId || originalBudget.eventId,
      data?.category || originalBudget.category,
      originalBudget.allocatedAmount,
