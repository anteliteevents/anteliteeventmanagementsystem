...
class PaymentService {
    ...
    async createPaymentIntent(reservationId, amount, currency = 'USD') {
        ...
        WHERE r.id = $1
      `;
            const reservationResult = await database_1.default.query(reservationQuery, [reservationId]);
        ...
        RETURNING *
      `;
            const transactionResult = await database_1.default.query(transactionQuery, [
                reservationId,
                amount,
                currency,
                paymentIntent.id,
            ...
            // Emit event
            await event_bus_1.eventBus.emit('payment.initiated', {
                reservationId,
    ...
    async confirmPayment(paymentIntentId) {
        ...
        RETURNING *
      `;
            const result = await database_1.default.query(updateQuery, [paymentIntentId]);
            ...
            // Emit events
            await event_bus_1.eventBus.emit('payment.completed', {
            ...
            // Emit failure event
            await event_bus_1.eventBus.emit('payment.failed', {
                paymentIntentId,
                error: error.message,
            });
            throw error;
     ...
     */
    async generateInvoice(reservationId) {
        try {
            // Check if invoice already exists
            const existingQuery = `
        SELECT * FROM invoices WHERE reservation_id = $1
      `;
            const existing = await database_1.default.query(existingQuery, [reservationId]);
        ...
        WHERE r.id = $1
      `;
            const details = await database_1.default.query(detailsQuery, [reservationId]);
        ...
        RETURNING *
      `;
            const invoice = await database_1.default.query(invoiceQuery, [
                reservationId,
                invoiceNumber,
                amount,
                taxAmount,
            ...
            await event_bus_1.eventBus.emit('invoice.generated', {
    ...
    `;
        const result = await database_1.default.query(query, [transactionId]);
    ...
    async processRefund(transactionId, amount) {
        try {
            ...
            const refund = await stripe_service_1.default.createRefund(transaction.stripePaymentIntentId, amount ? Math.round(amount * 100) : undefined);
        ...
        WHERE id = $1
      `;
            await database_1.default.query(updateQuery, [transactionId]);
            await event_bus_1.eventBus.emit('payment.refunded', {
                transactionId,
                refundId: refund.id,
